...
Interpolators VertexProgram(VertexData v)
{
    ...
    interpolators.normal = UnityObjectToWorldNormal(v.normal);
...
float4 FragmentProgram(Interpolators interpolators) : SV_TARGET
{
    ...
    float3 albedoTexture = tex2D(_AlbedoTex,interpolators.uv);
    float oneMinusReflectivity;
    
    //other way to correctly compute energy albedo *= 1 - max(_SpecularTint.r, max(_SpecularTint.g, _SpecularTint.b));
    //albedoTexture = EnergyConservationBetweenDiffuseAndSpecular(        albedoTexture, _SpecularTint.rgb, oneMinusReflectivity    );
                 
    return float4(albedoTexture + diffuse + specular,1);
}